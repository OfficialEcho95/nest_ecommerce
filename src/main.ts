...
async function bootstrap() {
  ...
  new RedisServer().getConnection(); // initialize redis connection early

  // 2️⃣ Load queues/workers AFTER Redis is ready
  const { QueuesAndWorkersStarter } = await import('./../queuesWorkers.start.js');
  new QueuesAndWorkersStarter();
